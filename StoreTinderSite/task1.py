...
conn = sqlite3.connect("kfcstore.sqlite")
c = conn.cursor()
c.execute("DROP TABLE IF EXISTS stores")
c.execute("""CREATE TABLE IF NOT EXISTS stores (storeID INTEGER PRIMARY KEY,
            latitude REAL, longitude REAL, storeName TEXT, storeAddress TEXT, storePhone TEXT CHECK(storePhone <> '') DEFAULT '-', 
            weekDayOpen TEXT, weekDayClose TEXT, weekEndOpen TEXT, weekEndClose TEXT)""")
for x in range(len(stores)):
    ...
    stores[x]["address"], stores[x]["phone"], stores[x]["weekdayopen"], stores[x]["weekdayclose"], 
    stores[x]["weekendopen"], stores[x]["weekendclose"]))
